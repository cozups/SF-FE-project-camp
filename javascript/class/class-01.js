...
/** 프로토타입이라는 것은 new 키워드를 통해 만든 생성자 함수에서 반환된 결과
 * 여기서 fruits라는 배열 데이터 혹은 인스턴스라고 일컫는다.
 *
 * 원시 타입, 참조 타입들도 마찬가지로 생성자 함수를 통해서 반환된 결과이고
 * 그동안 생성자 함수를 사용하지 않고, 각각의 데이터를 만든 것은 더 쉽게 만들 수 있게 각각의 리터럴 방식으로 만들었을 뿐이다.
 */
...
const user = {
  ...
  getFullName: function () {
    /** 객체 데이터 내부에서 일반 함수를 통해 메서드를 만들게 되면 this 키워드를 통해 접근할 수 있다. */
...
// 객체 데이터의 메서드를 일반 함수로 만들면 일반 함수에서의 this는 함수가 호출되는 곳에서 정의된다고 배웠습니다. 즉, user가 this가 됩니다.
console.log(user.getFullName());

/** 만약, user2 객체 데이터에서도 fullName을 반환하는 메서드를 만들어야 한다면 어떻게 할 수 있을까요?
 * 1. getFullName을 복사해서 user2 내부에 똑같이 만드는 방법
 * 1번 방식을 따르면정확하게 같은 로직을 가지고 있는 함수가 2번 만들어졌어요. 굉장히 비효율적
 *
